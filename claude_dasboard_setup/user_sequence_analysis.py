...
2.  In dashboard.py, add at the top:
        from user_sequence_analysis import (
            USER_SEQ_STORES,
            user_seq_tab_button,
            user_seq_tab_content,
            register_user_seq_callbacks,
        )
3.  In create_dashboard_layout():
    a.  Add USER_SEQ_STORES inside the Container (next to the other dcc.Store rows).
    b.  Add user_seq_tab_button() to the ButtonGroup alongside the other 3 tab buttons.
    c.  Add user_seq_tab_content() AFTER the TAB 3 (Epidemiology) Div.
4.  In the tab-switching callback (the one that sets display: none / block
    for overview-content, mutations-content, epidemiology-content), add:
        Output("user-seq-content", "style"),
        ...
        and return {"display": "none"} / {"display": "block"} accordingly.
5.  Call register_user_seq_callbacks(app) AFTER app is instantiated.

All identifiers are namespaced with "useq-" to avoid conflicts with
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    ...
                    # 4b: Phylogenetic tree + recombination side by side
                    dbc.Row([
                        dbc.Col([
                            dbc.Card([
                                dbc.CardBody([
...
def register_user_seq_callbacks(app):
    """
    Call this ONCE after your dash.Dash app is instantiated:

        app = dash.Dash(...)
        register_user_seq_callbacks(app)
    """
    ...
    def validate_sequences(n_clicks, upload_contents, textarea_value,
        ...
        4. Store validated sequences & enable Run button if no blocking errors.
        """
        ...
        return alert, store_data, btn_disabled
    ...
    @app.callback(
        ...
        Output("useq-progress-label",   "children"),
        Output("useq-progress-bar",     "value"),
        Output("useq-poll-interval",    "disabled"),
        Input("useq-btn-run", "n_clicks"),
        State("useq-validated-store", "data"),
        prevent_initial_call=True,
    )
    def start_analysis(n_clicks, validated_data):
        """
        Dispatches the analysis job.

        In production this should:
          1. Write the validated FASTA to a temp file keyed by job_id.
          2. Submit a Celery task (or Snakemake call) that runs:
               MAFFT → EPA-ng placement → IQ-TREE subtree → RDP5
          3. Return immediately so the UI stays responsive.

        The polling callback (below) then checks job status.
        """
        ...
        # Dispatch to the real pipeline (background thread via pipeline_runner)
        from pipeline_runner import dispatch_user_pipeline
        job_id = dispatch_user_pipeline(validated_data)

        return (
            "pending",
            job_id,
            {"display": "block"},           # show progress section
            "Submitting job…",
            5,
    ...
    def render_results(results):
        ...
        if not results:
            ...
            return empty, empty, go.Figure(), html.Div()
        ...
        # ── 7c: Sequence map (Plotly) ──
        fig = _build_sequence_map_figure(seqs, seq_map)

        # ── 7d: Phylogenetic tree container ──
        # We store the Newick in a data attribute; a clientside callback or
        # an embedded phylocanvas script picks it up.  Swap this Div for a
        # real phylo viewer integration (e.g. html.Iframe or dash-bio) as needed.
        phylo_div = html.Div(
            id="useq-phylo-newick-target",
            **{"data-newick": newick},
            children=[
                dbc.Alert(
                    [
                        html.I(className="bi bi-info-circle me-2"),
                        "Phylogenetic tree data ready. ",
                        html.Strong("Integration point: "),
                        "pass the Newick string in ",
                        html.Code("data-newick"),
                        " to your tree viewer (Phylocanvas.gl, dash-bio, "
                        "or an html.Iframe wrapping a D3 tree).",
                    ],
                    color="info",
                )
            ] if newick else [
        ...
        return summary_table, recombination_panel, fig, phylo_div
