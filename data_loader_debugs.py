...
def compute_burden_adjusted_coverage_from_sequences(
    ...
    """
    seq_df: hbv_data or hcv_data
            must contain Country_standard, genotype
    ihme_df: IHME dataframe already loaded
    virus: "HBV" or "HCV"
    """
    ...
    # --- Filter IHME to prevalence numbers ---
    ihme_prev = ihme_df[
        (ihme_df["cause"] == cause) &
    ...
    out["Coverage_ratio"] = out["Seq_count"] / out["Est_infections_genotype"]
...
def normalize_genotype_label(genotype: str, virus: str) -> str:
    """
    Normalize GLUE genotype labels to canonical dashboard keys.

    Examples:
      HBV: "Genotype A" -> "HBV-A"
...
def load_and_preprocess_data():
    ...
    try:
        ...
        print("HBV columns:", hbv_data.columns.tolist())
        print("HBV head:\n", hbv_data.head(3))
        print("HBV missing genotype:", hbv_data["genotype"].isna().mean() if "genotype" in hbv_data.columns else "NO genotype col")
        ...
        print(
        "Country overlap (HBV vs IHME):",
        len(set(hbv_data["Country_standard"]) & set(ihme_df["Country_standard"]))
        )
        ...
        print("HBV coverage rows:", len(hbv_cov))
        print("HCV coverage rows:", len(hcv_cov))
        print(hbv_cov.head(3))
